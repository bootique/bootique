...
my-app-1.0/
    ...
    lib/
        ...
        ...
```
This folder can be archived as a `.tar.gz` or `.zip`, to be distributed as a single file, and then unarchived on the
machine where the application needs to run. Creating such packaging with Maven involves `maven-jar-plugin`,
`maven-dependency-plugin` and `maven-assembly-plugin`. First let's create a folder structure:
