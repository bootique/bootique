...
=== BQRuntime and Dependency Injection
...
BQRuntime internally has an object called `Injector` that handles DI. It is a map of all reusable objects (often called
"services"). Registering in DI allows a service to simply declare all other services they need for their operation,
and the environment would find those dependencies and "inject" them in the objects.
...
At its core a module is simply a library of Java classes and resources with one extra special class implementing
`BQModule`. This class serves as an "assembly descriptor" for the objects in the library. It explains to the DI
environment how to create and configure them. Here is a simple module with a `configure` method (we'll explain `Binder`
a bit later):
...
By convention each module is packaged in a dedicated .jar file, though nothing would prevent you to keep more than one
module in a single .jar.
...
* It makes the constructed service available to be injected in other objects that depend on it
...
There are two ways to declare a service in a module - via explicit _bindings_ and with _provider_ methods. Bindings
are placed in the `configure()` method of the module:

[source,java,indent=0]
----
include::../../../../test/java/io/bootique/docs/programming/modules/BindingModule.java[tags=binding]
----
<1> Creates an association between `MyService` interface and a specific implementation. `MyService` can now be injected
into other services. A single instance of `MyServiceImpl` is created automatically by the DI container, its own
dependencies injected either via annotated constructor or into annotated fields as described in the injection section.

TODO...

=== Module Providers and Auto-Loading

Modules can be automatically loaded via `Bootique.autoLoadModules()` as long as they are present in the application
dependencies. Auto-loading uses Java
https://docs.oracle.com/javase/8/docs/api/java/util/ServiceLoader.html[ServiceLoader mechanism]. To ensure your modules
can be auto-loaded you need to do two things. First implement `io.bootique.BQModuleProvider` interface specific to
your module:
[source,java,indent=0]
----
include::../../../../test/java/io/bootique/docs/programming/modules/MyModuleProvider.java[tags=MyModuleProvider]
----
Second, create a file `META-INF/services/io.bootique.BQModuleProvider` with the only line being the name of your
BQModuleProvider implementor. E.g.:
...
`BQModuleProvider` has two more methods that you can optionally implement to help Bootique to make sense of the module
being loaded:
[source,java,indent=0]
----
...
Modules often depend to other "upstream" modules. They can interact with upstream modules in a few ways:
