...
Every now and then Bootique's own or your custom module become obsolete or replaced by other modules. There are two
interchangeable ways to tag them as "deprecated" to give a visual indication to the users to start looking for an
alternative. The first one is the standard Java `@Deprecated` annotation:

[source,java,indent=0]
----
include::../../../../test/java/io/bootique/docs/programming/modules/OldModule1.java[tags=deprecated]
----
The second is a `deprecated` flag on the `ModuleCrate`:

[source,java,indent=0]
----
include::../../../../test/java/io/bootique/docs/programming/modules/OldModule2.java[tags=deprecated]
----

Including any one of these modules in your app will result in an error message printed on startup to `STDERR`:

[source,terminaloutput]
----
** Deprecation alert - OldModule2.
----

You can improve this message to provide instructions to your users on how to replace the deprecated module using
`ModuleCrate.description(..)`:
[source,java,indent=0]
----
include::../../../../test/java/io/bootique/docs/programming/modules/OldModule2.java[tags=description]
----
This will print the following:
[source,terminaloutput]
----
** Deprecation alert - OldModule2: Deprecated and should be replaced with Module3.
----
...
via bindings and "provider" methods that are processed on startup by the dependency injection (DI) mechanism built into
Bootique.
...
Regardless of which flavor was selected, the implementation needs to be _bound_ in the module class:
[source,java,indent=0]
----
...
`Binder` allows to declare injectable collections (either Sets or Maps). Say you are writing your
own web server that will dispatch requests to Java servlets (of course `bootique-jetty` already provides such a server,
but it is still a good example):
[source,java,indent=0]
----
include::../../../../test/java/io/bootique/docs/programming/injection/SetInjectionModule.java[tags=set]
----
<1> Starts the binding of type `Set<Servlet>`
<2> Binds two servlets to the collection

Now the `Set<Servlet>` can be injected to our web server object:
[source,java,indent=0]
----
include::../../../../test/java/io/bootique/docs/programming/injection/SetInjectionModule.java[tags=injectSet]
----

A few notes on the code above:

* A unique signature of the Set (`Set<Servlet>`) is used as a key to identify it during injection. So something more
generic like `Set<String>` has a chance of multiple conflicting sets to be present in your app. To disambiguate them,
you should use `@Named` annotation.

* `Servlet1.class` and `Servlet2.class` are not simply classes, they are treated as DI keys to locate and
inject those objects into the `Set`. E.g., a dedicated "provides" method can be defined to create `Servlet2`:
+
[source,java,indent=0]
----
include::../../../../test/java/io/bootique/docs/programming/injection/SetInjectionModule.java[tags=provides]
----

`Binder.bindMap(..)` works similarly to `bindSet(..)` except, of course, it has injectable keys and values.

NOTE: Collection injection may seem complicated, but it is a very powerful mechanism that turns Bootique into a
 **plugin engine**, and modules - to "plugins". It allows "upstream" modules to define specific collections, and
"downstream" modules - add objects to those collections.

In fact the real servlet server in `bootique-jetty` does just that, but with a bit of syntactic sugar to hide some of
the complexity of DI collection binding:
[source,java,indent=0]
----
JettyModule.extend(binder)
   .addServlet(Servlet1.class)
   .addServlet(Servlet2.class);
----

The `addServlet(..)` method provides an convenient easy-to-understand API, but internally it still calls
`binder.bindSet(Servlet.class).add(..)`.

_TODO: collection bindings and advanced generics_
...
Most common commands are already available in various standard modules, still often you'd need to create your own.
To do that, first create a command class. It should implement `io.bootique.command.Command` interface,
though usually it more practical to extend `io.bootique.command.CommandWithMetadata` and provide some metadata used in
help and elsewhere:

[source,java,indent=0]
----
include::../../../../test/java/io/bootique/docs/programming/commands/MyCommand.java[tag=command]
----
...
[source,java,indent=0]
----
include::../../../../test/java/io/bootique/docs/programming/commands/MyModule.java[tag=addCommand]
----
...
[source,java,indent=0]
----
include::../../../../test/java/io/bootique/docs/programming/commands/MyDaemonCommand.java[tag=run]
----
...
[source,java,indent=0]
----
include::../../../../test/java/io/bootique/docs/programming/commands/InjectingCommand.java[tag=inject]
----
...
[source,java,indent=0]
----
include::../../../../test/java/io/bootique/docs/programming/commands/DecoratingModule.java[tag=decorate]
----
