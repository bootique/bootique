...
(`Bootique.app(args).module(MyModule.class)`), and this is highly impractical in real apps. So most often than not
the alternative autoloading mechanism is used. To enable autoloading, call `Bootique.autoLoadModules()` as a part of
the startup sequence. Also, each module needs an extra descriptor file called `io.bootique.BQModule` to be eligible for
autoloading. The file should be located on classpath under `META-INF/services` and should contain a line with the
fully-qualified name of the Module Java class. E.g.:
[source,text]
----
com.foo.MyModule
----

If you have more than one Bootique module in a given Java project, you should put the names of all modules in this file,
one name per line.
