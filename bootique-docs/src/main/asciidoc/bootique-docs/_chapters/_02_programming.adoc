...
Let's start with how you can leverage DI when creating your objects.

Services can annotate their fields and/or constructors with `@Inject` to indicate which internal variables they want
to be injected by the DI container. To demonstrate this we will show a couple of implementations of a simple "hello"
