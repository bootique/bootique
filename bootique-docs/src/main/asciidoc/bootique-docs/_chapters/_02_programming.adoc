...
----
Provider class has the same rules for injection of its own dependencies (either annotated fields or annotated
constructor).

The benefit of the provider is that it can implement custom assembly logic as mentioned above, which is separate from
the service itself. It also allows the service it creates to be completely free from injection annotations. Still often
creating a provider class is an overkill, and there is a simpler way to define small bits of assembly code -
...
----
<1> Does both assembly and binding of the Hello service.
<2> "configure" method can be empty, as the service was already bound via `provideHello` (or it can contain bindings for other
unrelated services)

A few notes on the `provideHello` method. It can be called anything. What makes it a provider is the `@Provides`
annotation. Its access modifier is also irrelevant (can be public, protected, etc.). As mentioned above this method
does both assembly and binding of the "Hello" service. The return type of the method (`Hello`) is used as the binding
key. And while there is no `@Inject` annotation in use anywhere, injection in fact also happens here - all method
arguments ("nameService" in this case) are assumed to be injectable and provided by the DI environment.
