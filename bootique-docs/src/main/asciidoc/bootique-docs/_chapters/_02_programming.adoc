...
<4> Exits the app passing the exit code that is the result of the execution to the OS.

This is it in a nutshell. Now let's discuss the details...

=== BQRuntime

`BQRuntime` is the object representing a Bootique app. It is the entry point to everything else. It holds all
other objects ("services"), configuration, commands, etc. The foundation of `BQRuntime` is _dependency injection_ (DI)
...
annotations from `javax.inject` package (https://jcp.org/en/jsr/detail?id=330[JSR-330]) are supported in Bootique.
But there are also quite a few differences.

Application code managed by `BQRuntime` comes via standard and custom "modules", so next let's discuss
what modules are.

=== Modules

Each module is a logical collection of (hopefully related) services with their assembly instructions. E.g. a standard
...
At its core, a module is simply a bunch of Java classes and resources. What makes it a module is a presence of a
...
NOTE: By convention, all classes belonging to a module are packaged in its own `.jar` file, though nothing prevents
