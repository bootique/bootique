...
Actually, there is a shorter version of `main(..)` that combines creation and execution in a single call to `exec()`:
[source,java,indent=0]
----
include::../../../../test/java/io/bootique/docs/programming/Application1.java[tag=Application]
----
That's what you'd normally do in a real app, but let's explore the `BQRuntime` object. It represents the entry
point of the application, holding all objects (aka "services") that were created by the `Bootique` builder. For all
intents and purposes, `BQRuntime` _is_ the "application". Here is how you can peek inside the runtime to access
its objects (here we are retrieving a `Cli` object that is defined in Bootique and describesb the command line arguments
...
In a Bootique app, most of your code (as well as framework code) is organized into "modules". Modules are regular
`.jar` files with some Java classes and resources logically related to each other in some way. Some examples from
Bootique are `bootique-jetty` that integrates Jetty server into your application, `bootique-jdbc` that allows to work
with relational databases, and so on.

What makes a `.jar` file a module is the presence of a special class implementing `BQModule`. This class provides a
"recipe" to Bootique on how to create objects ("services") from the classes present in the `.jar`. Creation of services
is assisted by the dependency injection (DI) mechanism of Bootique (discussed later), and all the services declared in a
given module in turn become available to be injected into other services.

The foundation of `BQRuntime` is _dependency injection_ (DI)
that glues things together.

NOTE: If you are familiar with other DI implementations, you will find a lot of similarities. Namely, standard
annotations from `javax.inject` package. But there are also quite a few differences.

An example of a simple module is shown below.
We'll explain the `configure(..)` method later.
...
NOTE: By convention, all classes belonging to a module are packaged in a separate `.jar` file, though nothing prevents
you from keeping multiple modules in a single `.jar`, or spreading the module code across multiple jars. Other than
autoloading descriptors explained below, there are no special packaging requirements for modules. When they are combined
in runtime, it is all a single classpath.
...
By default, for a module to be included in `BQRuntime` it has to be explicitly mentioned when starting a Bootique app
(`Bootique.app(args).module(MyModule.class)`), though this is rather impractical in real apps. Most often than not
you would rely on "autoloading" enabled via a call to `Bootique.autoLoadModules()` as a part of the startup
sequence. Additionally, for a module to be autoloadable, it needs a file called `io.bootique.BQModule`. This file should
be placed on classpath under `META-INF/services` and should contain a line with the fully-qualified name of the Module
Java class. E.g.:
[source,text]
----
com.foo.MyModule
----
If you have more than one module in a given project, you should put all the names in this file, one per line.

The result of autoloading, is that you can manage inclusion/exclusion of modules purely via the build system instead of in
the code.

NOTE: all standard Bootique modules are autoloadable.
...
As mentioned above, DI manages a subset of application objects that you'd write that we informally call "services".
Here is a rough list of DI functionality around services:
...
By convention, the application class implements `BQModule`. After all, a Bootique app is a collection
...
you may put the application class in `META-INF/services/io.bootique.BQModule` to make it auto-loadable and
remove this line.
...
More details on packaging are given in the "Assembly and Deployment" chapter.
