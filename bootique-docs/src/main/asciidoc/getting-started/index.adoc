...
:bq-header: getting-started/_data/header.html
...
Your app can just as easily obtain its own specific configuration in a form of an app-specific object, as described elsewhere in the docs.
...
As you see, we declared a variable of type `String[]` and annotated it with `@javax.inject.Inject` and `@io.bootique.annotation.Args`.
