...
----
include::../../../../test/java/io/bootique/getstarted/step1/Application.java[tags=all]
----
...
`--server` option looks promising, but before we use it, let's actually write a REST endpoint that will answer to our
requests. We'll use Jakarta https://en.wikipedia.org/wiki/Jakarta_RESTful_Web_Services[JAX-RS API] for that:

[source,java]
----
include::../../../../test/java/io/bootique/getstarted/step2/HelloResource.java[tags=all]
----

Note that we could have placed this code straight in the `Application` class. Which makes for an effective demo (an app
that can fit in one class), but not for a particularly clean design. So keeping the resource in its own class. Now
let's amend the `Application` to tell Bootique where to find the resource:

[source,java]
----
include::../../../../test/java/io/bootique/getstarted/step2/Application.java[tags=all]
----
...
Now when the app is started, you will see a different output:

[source]
----
INFO main i.b.j.s.ServletFactory: Adding servlet 'jersey' mapped to /*
INFO main i.b.j.s.ServerLifecycleLogger - Starting jetty...
INFO main o.e.j.s.Server - jetty-12.1.7; built: 2026-03-04T00:34:02.020Z; git: 8e2e9db9edceef93d37a63814ed06b921eb7f23d; jvm 21+35-LTS
...
...
* Declaring modules in the app dependencies and using `Bootique.autoLoadModules()` gives the app the ability to respond
to commands from those modules (in our exampl,  `--server` command coming from implicit `bootique-jetty` started an
embedded web server ).
