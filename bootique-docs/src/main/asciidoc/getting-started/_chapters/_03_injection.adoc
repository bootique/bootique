...
[source,java]
----
include::../../../../test/java/io/bootique/getstarted/step3/HelloResource.java[tags=all]
----
...
`@Inject` ensures that the value is initialized via injection, and `@Args` tells Bootique which one of possibly many
String[] instances from the DI container we are expecting here.

Now restart the app and refresh http://localhost:10001/hello[http://localhost:10001/hello] in the browser.
