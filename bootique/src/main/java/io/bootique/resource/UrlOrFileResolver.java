...
class UrlOrFileResolver {
    ...
    public static URL resolveSingle(String resourceId) {
        ...
        try {
            // using canonical file avoids downstream issues like https://github.com/bootique/bootique-jetty/issues/29
            return uri.isAbsolute() ? uri.toURL() : new File(resourceId).getCanonicalFile().toURI().toURL();
        } catch (IOException e) {
            throw new BootiqueException(1, "Invalid config resource url: " + resourceId, e);
        }
    }
